...
const MatchRing = ({ score }) => {
  ...
  // based on score
  const shadowColor = safeScore >= 70 ? 'rgba(16, 185, 129, 0.6)' : safeScore >= 40 ? 'rgba(245, 158, 11, 0.5)' : 'rgba(239, 68, 68, 0.5)';

  return (
    <div className="relative flex items-center justify-center w-12 h-12 flex-shrink-0">
      <svg className="w-full h-full transform -rotate-90" style={{ filter: `drop-shadow(0 0 8px ${shadowColor})` }}>
    ...
    >
      ...
      {isExpanded && (
        <div className="px-6 pb-6 pt-2 border-t border-slate-50">
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2">
                {app.pros?.length > 0 ? app.pros.map((p, i) => (
                  <li key={i} className="text-xs text-slate-600 font-medium flex items-start gap-2 opacity-0 animate-slideup" style={{ animationDelay: `${i * 0.1}s` }}>
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2">
                {app.cons?.length > 0 ? app.cons.map((c, i) => (
                  <li key={i} className="text-xs text-slate-600 font-medium flex items-start gap-2 opacity-0 animate-slideup" style={{ animationDelay: `${i * 0.1 + 0.2}s` }}>
          ...
          {/* actions for applications inside expansion */}
          <div className="flex items-center justify-end gap-2 mt-4 pt-4 bg-white rounded-xl p-3">
